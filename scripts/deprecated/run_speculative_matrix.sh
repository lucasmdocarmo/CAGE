#!/bin/bash
# =============================================================================
# DEPRECATED (2026-08-02): speculative arms are RETIRED from the campaign design
# per the publication charter §7.5 (MyDocs/PUBLICATION.md). Kept ONLY so the
# pilot runs that used it stay reproducible. Do NOT run against campaign data.
# See scripts/deprecated/README.md.
# =============================================================================
...
#
# Usage (DEPRECATED): bash scripts/deprecated/run_speculative_matrix.sh [MODEL]
#   MODEL defaults to Qwen/Qwen3-8B. Run ONCE PER MODEL to cover both speculative families:
#     bash scripts/deprecated/run_speculative_matrix.sh Qwen/Qwen3-8B
#     bash scripts/deprecated/run_speculative_matrix.sh XiaomiMiMo/MiMo-7B-RL
#   Defaults to the full design (500 queries x 3 trials); override for a smoke, e.g.
#     NUM_QUERIES=5 NUM_TRIALS=1 bash scripts/deprecated/run_speculative_matrix.sh <MODEL>
#
...
# VLLM_ENFORCE_EAGER=1 bash scripts/deprecated/run_speculative_matrix.sh <model> -- a recorded deviation
...
if [ "$SKIP_SPEC_GATE" = "1" ]; then
  ...
  bash scripts/deprecated/check_mtp_spec_decode.sh "$MODEL" "$DRAFT"; _grc=$?
  if [ "$_grc" = "2" ]; then
