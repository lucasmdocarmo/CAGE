...
scripts/
  ...
  2_serving/        start / manage the engines         manage_vllm_server.sh  manage_sglang_server.sh  manage_lmdeploy_server.sh  manage_vllm_pd.sh  pd_proxy.py  manage_vllm_cluster.py  deploy_cluster.sh
...
| `2_serving/manage_vllm_pd.sh` | stage 2 — after 1_setup, before the pd-topology 3_run cells (DIST overlay); re-run per pd relaunch | Start/stop/status the intra-node vLLM prefill/decode disaggregation stack (2 role instances + pd_proxy.py) under the uniform serving regime | both |
...
| `2_serving/pd_proxy.py` | stage 2 — launched by manage_vllm_pd.sh after both role instances are ready; clients hit THIS port | Stdlib-only prefill/decode disaggregation front-end (1P1D pattern: prefill request with max_tokens=1, then the full request to decode; streaming passthrough) | both |
...
| `3_run/run_campaign.py` | stage 3 — THE campaign sweep driver (tranche P1, audit gap G1); 'plan' runs locally before any provisioning, 'run' executes on the pod after 2_serving; ends by invoking seal_campaign_run.py | Enumerate ONE session's registered D6 grid into a reviewable execution-plan JSON ('plan', pure), then execute the plan cell-by-cell with engine-relaunch boundaries, per-window resume and fail-continue ('run') | both |
...
| `4_analysis/build_floor_table.py` | preflight — BEFORE calibrate_cell and any campaign dispatch | Produce the P6 floor table: per-cell-family PRE-MEASUREMENT predictions (demand D, KV-side saturation bound, predicted collapse onset) persisted for the ±15% falsification join; refuses overwrite without --force | local |
...
| `4_analysis/build_retrieval_gate_table.py` | preflight — charter-MANDATORY BEFORE any campaign dispatch (§7.2(a)) | Produce the §7.2(a) offline retrieval gate table (bm25 vs dense vs rrf, first-stage pool recall@100) that decides registered contrast #6; refuses overwrite without --force | local |
...
| `4_analysis/ragas_head_to_head.py` | stage 4 — offline, over a quality-SCORED run's rows (never in the serving loop) | §8.6(d) RAGAS head-to-head runner — the three-way faithfulness table (LettuceDetect vs AlignScore vs pinned LLM judge) | local |
...
| `4_analysis/yield_ladder.py` | stage 4 helper — consumed by run_campaign_analysis.py | the S1 yield ladder + independence null made VISIBLE (T6.2) | local |
