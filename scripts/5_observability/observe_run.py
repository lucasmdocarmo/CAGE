...
def main() -> int:
    ...
    ap.add_argument("--run-dir", default=os.environ.get("CAGE_RUN_ROOT", "results"),
                    help="Run output tree to observe/mirror (default: $CAGE_RUN_ROOT, else results).")
    ap.add_argument("--run-id", default=os.environ.get("CAGE_RUN_ID"),
                    help="Run-id recorded in the manifest (default: $CAGE_RUN_ID); unifies the "
                         "manifest run_id with the directory run-id.")
    ...
    run_id = args.run_id or f"{_slug(args.model or 'run')}_{created_at.replace(':', '').replace('-', '')[:15]}"
