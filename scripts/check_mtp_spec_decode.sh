...
# Usage: bash scripts/check_mtp_spec_decode.sh <MODEL> '<spec_json>'
#    or: VLLM_SPECULATIVE_CONFIG='<spec_json>' bash scripts/check_mtp_spec_decode.sh <MODEL>
# =============================================================================
set -uo pipefail
...
if ! VLLM_SPECULATIVE_CONFIG="$SPEC" "$SCRIPT_DIR/manage_vllm_server.sh" restart "$MODEL"; then
